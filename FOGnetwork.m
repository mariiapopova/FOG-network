...
gsyn = [1 0.3 1.2 0.3 1 .08]; Esyn = [-85 0 -85 0 -85 -85]; %alike in Rubin gsyn and in So Esyn
%gsynppn = [0.15 0.1 0.15]; Esynppn = [0 -95 0]; %alike in Lourens
gsynppn = [0.12 0.06 0.12 0.06]; Esynppn = [0 -85 0 0]; %in order snppn gippn ppnsn ppngi
tau=5; gpeak1=0.3; gpeak=0.43; %parameters for second-order alpha synapse
gsynsnr=0.3; Esynsnr=0; %for snr synapses in order stn
gsynstr=[0.5 0.5 0.5]; ggaba=0.1; gcorstr=0.07; Esynstr=[-80 0 -85 -85 -85]; tau_i=13;%parameters for striatum synapses in order gaba-rec
...
for i=2:length(t)
    ...
    %Igege=0.5*((gsyn(3)-0.2*pd)*(V3-Esyn(3)).*(S31+S32)); %first-order kinetics 1ge to 2ge
    Igege=0.5*(gsyn(3)*(V3-Esyn(3)).*(S31+S32)); %first-order kinetics 1ge to 2ge
    Iappgpe=21-13*pd+r; %why need r??   
    %Iappgpe=2+0.2*pd; %rubin
    %Iappgpe=3; %kumaravelu
    %str-gpe synapse
    ...
    %Icorstr=(1-0.044*pd)*gcorstr*(V7-Esynstr(2)).*Sc(:,i-1);
    Icorstr=gcorstr*(V7-Esynstr(2)).*Sc(:,i-1);
    %ge-str synapse
    ...
    %PPN
    vppn(:,i)=V5+dt*(1/Cm*(-Inal5-Ikl5-Ina5-Ik5-It5-Ihyp5-Inap5+Iappppn+Icort-Isnppn-Igippn));
    H5=H5+dt*((h5-H5)./th5);
    M5=M5+dt*((m5-M5)./tm5);
