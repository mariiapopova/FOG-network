...
%pd params 26.08
aopt=linspace(4,7,5);
bopt=linspace(1*0.044,6*0.044,5);
copt=linspace(0.1,2.5,5);
dopt=linspace(3,9,5);
...
h=FOGnetwork(0,0,0,aopt(i), bopt(j), copt(k), dopt(f)); %healthy
pd = FOGnetwork(1,0,0,aopt(i), bopt(j), copt(k), dopt(f)); %PD
...
new_row_of_values = [aoptdum, boptdum, coptdum, doptdum, h, pd];
